...
import { BrixDetailPopup, type SelectedImage } from '@/components/shared';
...
type DomeGalleryProps = {
    ...
    grayscale?: boolean;
    /** Popup mode: 'brix' for BRIX detail popup, 'enlarge' for original enlarged image */
    popupMode?: 'brix' | 'enlarge';
};
...
export default function DomeGallery({
            ...
            onDrag: ({
                ...
                if (last) {
                    ...
                    if (isTap && tapTargetRef.current && !focusedElRef.current) {
                        if (popupMode === 'brix') {
                            // Get image data from parent element
    ...
    const openItemFromElement = (el: HTMLElement) => {
        ...
        img.src = rawSrc;
        img.alt = rawAlt;
        // Show full color when enlarged (no grayscale)
        img.style.cssText = `width:100%; height:100%; object-fit:cover;`;
  ...
  `;
    ...
    return (
        <>
                                    ...
                                    <div
                                        ...
                                        onClick={(e) => {
                                            ...
                                            if (performance.now() - lastDragEndAt.current < 80)
                                                return;
                                            if (popupMode === 'brix') {
                                                // Open BRIX detail popup
                                                setSelectedImage({ src: it.src, alt: it.alt });
                                            } else {
                                                // Open original enlarged image
                                                if (openingRef.current) return;
            ...
            {/* BRIX Detail Popup */}
            <BrixDetailPopup image={selectedImage} onClose={() => setSelectedImage(null)} />
        </>
    );
}
