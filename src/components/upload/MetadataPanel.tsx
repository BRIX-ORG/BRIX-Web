...
export function MetadataPanel({ fileData, isProcessing = false }: MetadataPanelProps) {
    const t = useTranslations('uploads.analysis');
    return (
        <div className="bg-muted border border-border flex flex-col h-full min-h-125">
            ...
            <div className="flex-1 p-4 font-mono text-[11px] overflow-y-auto space-y-4">
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex justify-between border-b border-border/50 py-1">
                        <span className="text-muted-foreground">{t('bitDepth')}:</span>
                        <span className="text-foreground/80">{fileData?.bitDepth || '--'}</span>
                    </div>
                    <div className="flex justify-between border-b border-border/50 py-1">
                        <span className="text-muted-foreground">{t('colorSpace')}:</span>
                ...
                <div className="space-y-1 pt-4">
                    ...
                    <div className="bg-black/50 p-3 text-muted-foreground leading-relaxed break-all">
                        ...
                        <p className="mt-2 text-primary/40">
                            {t('genHash')}: {fileData?.hash || t('waiting')}
                        </p>
                        <p className="text-primary/40">
                            {t('timestamp')}: {fileData?.timestamp || '--'}
                        </p>
                        <p className="text-primary/40">
                            {t('geoTag')}: {fileData?.geoTag || '--'}
                        </p>
                        <p className="mt-2 text-secondary/40">
                            {t('aiProbability')}: {fileData?.aiProbability || '--'}
                        </p>
                        <p className="text-secondary/40">
                            {t('artifactScan')}: {t('noAnomalies')}
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-1 pt-4">
                    ...
                    <p className="text-[10px] text-muted-foreground/60 italic">
                        {isProcessing ? t('processing') : t('waiting')}
                    </p>
                    <p className="text-[10px] text-muted-foreground/60 italic">{t('listening')}</p>
            ...
            <div className="p-4 bg-black/40 border-t border-border">
                <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-[10px] font-bold text-primary">{t('pending')}</span>
                </div>
